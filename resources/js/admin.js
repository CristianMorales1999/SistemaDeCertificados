...
function initializeDashboard() {
    ...
    dashboardIndicador.classList.add('bg-accent-300');
...
function showSection(sectionId) {
    ...
    const indicadores = document.querySelectorAll('.flex.items-center > div:first-child');
    indicadores.forEach(indicador => {
        indicador.classList.remove('bg-accent-300', 'bg-accent-100');
        indicador.classList.add('bg-white');
    });
    ...
    activeIndicador.classList.add('bg-accent-300');

    // Resetear estilos de todos los botones
    const navButtons = document.querySelectorAll('.nav-button, .active-nav-button');
    navButtons.forEach(button => {
        button.classList.remove('active-nav-button');
        button.classList.remove('bg-accent-300');
        button.classList.remove('hover:bg-primary-300');
        button.classList.add('bg-white');
        button.classList.add('hover:bg-accent-50');
        const text = button.querySelector('h1');
        if (text) {
            text.classList.remove('text-white');
            text.classList.add('text-neutral-900');
        }
    });
    ...
    if (activeButton) {
        ...
        activeButton.classList.remove('hover:bg-accent-50');
        activeButton.classList.add('bg-accent-300');
        activeButton.classList.add('hover:bg-primary-300');
        const text = activeButton.querySelector('h1');
        if (text) {
            text.classList.remove('text-neutral-900');
